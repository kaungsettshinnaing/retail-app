import { prisma as db } from "@/lib/db";
import { formatDate, formatMoney } from "@/lib/format";
import { requireSession } from "@/lib/auth";
import { accountingDict } from "@/lib/i18n/dict/accounting";
import PaymentForm from "./PaymentForm";

export const dynamic = "force-dynamic";

const DAY_MS = 24 * 60 * 60 * 1000;

function agingBadge(
  invoiceDate: Date,
  t: { agingOverdue: string; agingDueSoon: string; agingCurrent: string },
): { label: string; style: string } {
  const days = Math.floor((Date.now() - invoiceDate.getTime()) / DAY_MS);
  if (days > 30) return { label: `${t.agingOverdue} (${days}d)`, style: "bg-red-100 text-red-700" };
  if (days >= 23) return { label: `${t.agingDueSoon} (${days}d)`, style: "bg-amber-100 text-amber-700" };
  return { label: `${t.agingCurrent} (${days}d)`, style: "bg-gray-100 text-gray-600" };
}

export default async function PayablePage() {
  const user = await requireSession();
  const t = accountingDict[user.language];

  const invoices = await db.supplierInvoice.findMany({
    where: { paidAt: null, status: { not: "DRAFT" } },
    orderBy: { invoiceDate: "asc" },
    include: { supplier: { select: { name: true } } },
  });

  return (
    <div className="space-y-4">
      <h1 className="section-title">{t.payableTitle}</h1>
      <div className="card overflow-hidden p-0">
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
              <th className="py-2 px-3 text-left">{t.colInvoice}</th>
              <th className="py-2 px-3 text-left">{t.colSupplier}</th>
              <th className="py-2 px-3 text-left">{t.colInvoiceDate}</th>
              <th className="py-2 px-3 text-left">{t.colAging}</th>
              <th className="py-2 px-3 text-right">{t.colAmount}</th>
              <th className="py-2 px-3 text-right">{t.colAction}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {invoices.length === 0 && (
              <tr>
                <td colSpan={6} className="py-8 text-center text-sm text-gray-400">
                  {t.noUnpaidInvoices}
                </td>
              </tr>
            )}
            {invoices.map((inv) => {
              const aging = agingBadge(inv.invoiceDate, t);
              return (
                <tr key={inv.id}>
                  <td className="py-2 px-3 text-sm font-medium text-gray-800">{inv.invoiceNo ?? inv.id.slice(0, 8)}</td>
                  <td className="py-2 px-3 text-sm text-gray-600">{inv.supplier?.name ?? "—"}</td>
                  <td className="py-2 px-3 text-sm text-gray-500">{formatDate(inv.invoiceDate)}</td>
                  <td className="py-2 px-3">
                    <span className={`badge ${aging.style}`}>{aging.label}</span>
                  </td>
                  <td className="py-2 px-3 text-sm text-right">{inv.totalAmount != null ? formatMoney(inv.totalAmount) : "—"}</td>
                  <td className="py-2 px-3">
                    <PaymentForm invoiceId={inv.id} lang={user.language} />
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </div>
  );
}
