...
export function CreateAdvanceForm({
  ...
  return (
    <div className="card">
      <h2 className="section-title text-sm mb-3">{t.addSalaryAdvanceTitle}</h2>
      <form action={handleSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <input name="note" placeholder={t.notePlaceholder} className="input" disabled={pending} />
        ...
        <button type="submit" className="btn-primary sm:col-span-2 lg:col-span-5" disabled={pending}>
          {pending ? c.saving : t.addAdvanceBtn}
        </button>
      </form>
...
export function DeleteInstalmentButton({ id, lang }: { id: string; lang: Language }) {
  const c = commonDict[lang];
  const [pending, startTransition] = useTransition();
  const router = useRouter();
