...
export function CreateFineForm({ employees }: { employees: { userId: string; name: string }[] }) {
  ...
  return (
    <div className="card">
      <h2 className="section-title text-sm mb-3">Add Fine</h2>
      <form action={handleSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <input name="reason" required placeholder="Reason" className="input" disabled={pending} />
...
export function DeleteFineButton({ id }: { id: string }) {
  const [pending, startTransition] = useTransition();
  const router = useRouter();
